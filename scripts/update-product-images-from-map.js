...
const DEFAULT_MAP_PATH = path.join(__dirname, '..', 'Images', 'loyverse', 'menu', 'image-map.json');
...
async function updateItem(token, item, dryRun) {
  ...
  const currentName = fieldString(fields, 'name') || fieldString(fields, 'nameTh') || '';
  const currentUrl = fieldString(fields, 'imageUrl');
  if (normalizeName(currentName) !== normalizeName(item.name)) {
    return { id: item.id, name: item.name, currentName, status: 'skipped', reason: 'name-mismatch' };
  }
  ...
  params.append('updateMask.fieldPaths', 'loyverseImageUrl');
  ...
  const update = await requestJson(`${url}?${params.toString()}`, {
    ...
    body: JSON.stringify({
      fields: {
        ...
        loyverseImageUrl: { stringValue: item.sourceImageUrl || '' },
...
async function main() {
  ...
  const mapPath = mapArgIndex >= 0 && process.argv[mapArgIndex + 1] ? path.resolve(process.argv[mapArgIndex + 1]) : DEFAULT_MAP_PATH;
  const map = readJson(mapPath);
  const items = Array.isArray(map.items) ? map.items : [];
  if (!items.length) throw new Error(`No items found in ${mapPath}`);
