    ...
    return `
        <div class="eden-skeleton-form" aria-hidden="true">
            ${Array.from({ length: rows }, (_, index) => `
                <span>
                    ${skeletonLine(index % 2 ? 'medium' : 'short')}
                    <span class="eden-skeleton-input"></span>
                </span>
            `).join('')}
        </div>
    `;
...
function profileSkeleton() {
    return `
        <div class="eden-skeleton-profile" aria-hidden="true">
            ...
            <section class="eden-skeleton-grid stats">
                ${Array.from({ length: 3 }, compactCardSkeleton).join('')}
            </section>
            <section class="eden-skeleton-panel">
                ${formSkeleton(4)}
            </section>
            <section class="eden-skeleton-grid cards">
                ${Array.from({ length: 4 }, compactCardSkeleton).join('')}
            </section>
        </div>
    `;
}
...
function summarySkeleton(rows = 5) {
    return `
        <div class="eden-skeleton-summary" aria-hidden="true">
            ${Array.from({ length: rows }, () => `
                <span class="eden-skeleton-summary-row">
                    ${skeletonLine('short')}
                    ${skeletonLine('medium')}
                </span>
            `).join('')}
