...
import { Badge, Panel, Skeleton, SkeletonGrid } from '@/components/ui';
    ...
    <main className="px-5 py-10">
      <div className="mx-auto max-w-6xl">
        <section className="rounded-lg border border-line bg-white p-5 shadow-panel">
          ...
          {loading ? <Skeleton className="mt-2 h-11 w-64 max-w-full" /> : <h1 className="mt-2 text-4xl font-black">{author?.name || filtered[0]?.author_name || 'ผู้เขียน'}</h1>}
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          {!loading && !filtered.length && <Panel className="md:col-span-2 xl:col-span-3">ยังไม่มีบทความ published จากผู้เขียนนี้</Panel>}
        </div>
      </div>
    </main>
  );
