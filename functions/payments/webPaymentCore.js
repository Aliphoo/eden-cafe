...
function sourceContact(source = {}) {
  return {
    ...
    customer_phone: contactPhone(
      ...
      source.phone,
      source.checkoutPhone,
      source.checkout_phone,
      source.contactPhone,
      source.contact_phone
    ),
    customer_email: firstClean(
      180,
...
function profileContact(user = {}, summary = {}) {
  return {
    customer_name: firstClean(
      120,
      user.displayName,
...
async function readMemberContact(transaction, db, memberId) {
  const safeMemberId = cleanString(memberId, 180);
...
async function enrichContextContact(transaction, db, context = {}) {
  const source = {
    customer_name: context.customer_name || '',
    customer_phone: context.customer_phone || '',
    customer_email: context.customer_email || '',
  };
  if (source.customer_phone && source.customer_name && source.customer_email) {
    return { ...context, ...source };
  }
  const fallback = await readMemberContact(transaction, db, context.member_id);
  return {
    ...context,
    customer_name: source.customer_name || fallback.customer_name || '',
    customer_phone: source.customer_phone || fallback.customer_phone || '',
    customer_email: source.customer_email || fallback.customer_email || '',
  };
}
...
function normalizeShopCategoryIds(product = {}) {
  const ids = [];
  const add = value => {
    ...
    if (value && typeof value === 'object') {
      add(value.id || value.categoryId || value.category_id || value.slug || value.name);
      return;
    }
  ...
  add(product.categoryIds);
  add(product.category_ids);
  add(product.categories);
  return Array.from(new Set(ids));
}

function shopProductCategoryName(product = {}) {
  return cleanString(
...
async function buildShopOrderLines(transaction, db, items) {
  ...
  for (const item of items) {
    ...
    const categoryIds = normalizeShopCategoryIds(product);
    ...
    lines.push({
      ...
      categoryId,
      categoryIds,
      categoryName: shopProductCategoryName(product),
...
async function prepareCheckoutPhoneProfileSync(transaction, db, uid, phone) {
...
function writePreparedCheckoutPhoneProfileSync(transaction, prepared = {}) {
  if (prepared.synced === true && prepared.ref && prepared.patch) {
    transaction.set(prepared.ref, prepared.patch, { merge: true });
  }
}

async function syncCheckoutPhoneToProfile(transaction, db, uid, phone) {
  const prepared = await prepareCheckoutPhoneProfileSync(transaction, db, uid, phone);
  writePreparedCheckoutPhoneProfileSync(transaction, prepared);
  return {
    synced: prepared.synced === true,
...
function sourceContextFromSnapshot(sourceType, sourceId, branchId, ref, data) {
  ...
  const contact = sourceContact(data);
  return {
    ref,
    data,
...
async function createBeamPaymentLink(config, context, paymentId, idempotencyKey, returnUrl = '') {
  ...
  const collectPhoneNumber = !context.customer_phone;
  const body = {
...
function pendingSourceUpdate(context, paymentId, provider, paymentEnv) {
  ...
  if (context.customer_name) {
    update.customer_name = context.customer_name;
    if (!context.data?.name) update.name = context.customer_name;
  }
  if (context.customer_phone) {
    update.customer_phone = context.customer_phone;
    update.phone = context.customer_phone;
  }
  if (context.customer_email) {
    update.customer_email = context.customer_email;
  }
  if (context.source_type === 'SHOP_ORDER') {
...
function sourcePaidUpdate(context, paymentId, providerRef) {
  ...
  if (context.source_type === 'ARCHERY_BOOKING') {
    ...
    const promoApplications = promotionApplicationsWithStatus(context.data, 'redeemed', { paymentId });
    if (promoApplications.length) {
      update.promoApplications = promoApplications;
      update.promo_applications = promoApplications;
      update.promoApplicationsUpdatedAt = now;
    }
  }
  return update;
}

function sourcePaymentOnlyUpdate(status, context = null) {
  const now = FieldValue.serverTimestamp();
  ...
  if (context?.source_type === 'SHOP_ORDER' && ['FAILED', 'CANCELLED'].includes(status)) {
    const promoApplications = promotionApplicationsWithStatus(context.data, 'released', { reason: status });
    if (promoApplications.length) {
      update.promoApplications = promoApplications;
      update.promo_applications = promoApplications;
      update.promoApplicationsUpdatedAt = now;
...
function setPaymentBase(context, paymentId, providerResult, provider, idempotencyKey) {
  ...
  return {
    ...
    customer_phone: context.customer_phone || '',
    customer_email: context.customer_email || '',
    collect_phone_number: providerResult.collect_phone_number,
...
function normalizeShopPromoCode(value) {
  ...
  return Array.isArray(value)
    ? value.map(row => ({
      ...
      categoryIds: Array.isArray(row.categoryIds || row.category_ids)
        ...
        : [],
      archeryItem: cleanString(row.archeryItem || row.archery_item, 80),
...
function promotionApplicationFromReservation(promoCode, reservation = {}) {
  const code = normalizeShopPromoCode(reservation.code || promoCode);
...
function shopPromotionApplications(source = {}) {
  ...
  return raw
    .map(application => {
      const code = normalizeShopPromoCode(application.code || application.promoCode || application.promo_code);
...
function promotionApplicationsWithStatus(source = {}, status = '', details = {}) {
  ...
  return raw.map(application => ({
    ...
    receiptId: details.receiptId || application.receiptId || application.receipt_id || '',
    receipt_id: details.receiptId || application.receipt_id || application.receiptId || '',
    releaseReason: details.reason || application.releaseReason || application.release_reason || '',
  }));
}

async function commitShopPromotionApplications(transaction, db, context, options = {}) {
  if (context.source_type !== 'SHOP_ORDER') return [];
  const applications = shopPromotionApplications(context.data);
  const results = [];
  for (const application of applications) {
    results.push(await promotionsEngine.commitPromotionRedemptionInTransaction(transaction, db, {
      ...
      source_id: context.source_id,
      payment_id: options.paymentId || options.payment_id || '',
      receipt_id: options.receiptId || options.receipt_id || '',
    }, {
...
async function releaseShopPromotionApplications(transaction, db, context, options = {}) {
  if (context.source_type !== 'SHOP_ORDER') return [];
  const applications = shopPromotionApplications(context.data);
  const results = [];
  for (const application of applications) {
...
const createShopOrderDraft = httpFunction(async ({ db, data, actor, requestId }) => {
  ...
  const promoCode = normalizeShopPromoCode(data.promo_code || data.promoCode);
  ...
  const result = await runIdempotentTransaction(db, {
    ...
    const profilePhoneSync = await prepareCheckoutPhoneProfileSync(transaction, db, actor.uid, phone);
    if (orderSnap.exists) {
      ...
      writePreparedCheckoutPhoneProfileSync(transaction, profilePhoneSync);
      return {
        source_type: 'SHOP_ORDER',
        source_id: orderRef.id,
        order_id: existing.id || existing.order_id || orderRef.id,
        branch_id: existing.branch_id || branchId,
        amount: Number(existing.totalAmount || existing.total || existing.amount || 0) || 0,
        ...
        promo_applications: Array.isArray(existing.promo_applications) ? existing.promo_applications : (existing.promoApplications || []),
        totals: {
          subtotal: Number(existing.subtotal || 0) || 0,
          discount: Number(existing.discount || existing.discount_total || 0) || 0,
          shippingFee: Number(existing.shippingFee || existing.shipping_fee || 0) || 0,
          totalAmount: Number(existing.totalAmount || existing.total || existing.amount || 0) || 0,
        },
      };
    }

    let promoApplication = null;
    let discount = 0;
    if (promoCode) {
      ...
      promoApplication = promotionApplicationFromReservation(promoCode, reservation);
      if (!promoApplication) throw apiError('PROMO_CODE_INVALID', 409, 'Promo code is invalid');
      discount = promoApplication.discountAmount;
    }
    const promoApplications = promoApplication ? [promoApplication] : [];
    const totalBeforeDiscount = roundMoney(subtotal + shippingFee);
    const totalAmount = roundMoney(Math.max(0, totalBeforeDiscount - discount));
...
const createPaymentIntent = httpFunction(async ({ db, data, actor, requestId }) => {
  ...
  const precheck = await db.runTransaction(async transaction => {
    let context = await readSource(transaction, db, branchId, sourceType, sourceId);
    context = await enrichContextContact(transaction, db, context);
...
const cancelPendingPayment = httpFunction(async ({ db, data, actor, requestId }) => {
  ...
  const result = await runIdempotentTransaction(db, {
    ...
    await releaseShopPromotionApplications(transaction, db, context, {
      actor,
      requestId,
      reason,
    });
    ...
    transaction.update(context.ref, sourcePaymentOnlyUpdate('CANCELLED', context));
    writeAuditLog(transaction, db, {
      branchId,
      actor,
      actorType: actor.uid === context.member_id ? 'CUSTOMER' : 'STAFF',
...
const approveRefund = httpFunction(async ({ db, data, actor, requestId }) => {
  ...
  const result = await runIdempotentTransaction(db, {
    ...
    if (markRefunded) transaction.update(context.ref, sourcePaymentOnlyUpdate(paymentRefundStatus, context));
...
const issueWebReceipt = httpFunction(async ({ db, data, actor, requestId }) => {
  ...
  const result = await runIdempotentTransaction(db, {
    ...
    const sourceId = cleanString(payment.data.source_id || payment.data.booking_id || payment.data.order_id, 180);
    const context = await readSource(transaction, db, branchId, sourceType, sourceId);
    const receiptId = paymentDocId('web_receipt', payment.data.payment_id || payment.data.id || context.source_id);
    await commitShopPromotionApplications(transaction, db, context, {
  ...
  try {
    ...
    const result = await db.runTransaction(async transaction => {
      ...
      if (SUCCESS_EVENTS.has(eventType)) {
        ...
        const receiptId = paymentDocId('web_receipt', paidPayment.payment_id || paidPayment.id || context.source_id);
        await commitShopPromotionApplications(transaction, db, context, {
          paymentId: payment.ref.id,
          receiptId,
      ...
      transaction.update(context.ref, sourcePaymentOnlyUpdate(failedStatus, context));
      transaction.set(webhookEventRef, {
        ...base,
        status: failedStatus,
        updated_at: FieldValue.serverTimestamp(),
