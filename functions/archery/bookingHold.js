...
function buildBookingPayload(options = {}) {
  ...
  const discount = Number(pricing.discount_total || pricing.discount || 0) || 0;
  const totalBeforeDiscount = Number(pricing.total_before_discount || pricing.subtotal_amount || amount + discount) || amount;
  ...
  return {
    ...
    subtotal_amount: totalBeforeDiscount,
    discount,
    ...
    promoCode: pricing.promoCode || pricing.promo_code || '',
    promoApplications: pricing.promoApplications || [],
    promo_applications: pricing.promo_applications || pricing.promoApplications || [],
    promotionRedemptionIds: pricing.promotionRedemptionIds || [],
    promotionLineAllocations: pricing.promotionLineAllocations || [],
    promotion_line_allocations: pricing.promotion_line_allocations || pricing.promotionLineAllocations || [],
...
async function createArcheryBookingInTransaction(transaction, db, options = {}) {
  ...
  const expiresAt = options.bookingStatus === 'HELD'
    ...
    : null;
  const promotion = await reserveArcheryPromotionInTransaction(transaction, db, {
...
const createArcheryHold = httpFunction(async ({ db, data, actor, requestId }) => {
  ...
  const promoCode = archeryPromoCode(data.promo_code || data.promoCode);
  ...
  const result = await runIdempotentTransaction(db, {
    ...
    return {
      ...
      totalAmount: created.booking.totalAmount,
      amount_before_discount: created.booking.amount_before_discount,
      total_before_discount: created.booking.total_before_discount,
      subtotal_amount: created.booking.subtotal_amount,
      discount: created.booking.discount,
      discount_total: created.booking.discount_total,
      promo_code: created.booking.promo_code,
      promoCode: created.booking.promoCode,
      promoApplications: created.booking.promoApplications,
      promo_applications: created.booking.promo_applications,
      promotionLineAllocations: created.booking.promotionLineAllocations,
      promotion_line_allocations: created.booking.promotion_line_allocations,
