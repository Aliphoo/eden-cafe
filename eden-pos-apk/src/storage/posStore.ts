...
import { receiptReportDateKey } from "../domain/receiptDates";
...
const hydrateReceipt = (receipt: Receipt): Receipt => {
  const isTestOrder = receipt.isTestOrder === true;
  const softLaunch = receipt.softLaunch === true;
  const loyaltySkipReason: LoyaltySkipReason | "" =
    receipt.loyaltySkipReason ||
    (isTestOrder ? "test-order" : softLaunch ? "soft-launch" : "");
  const isPaid =
  ...
  const openedAt = receipt.openedAt || receipt.createdAt || createdAt;
  const paidAt =
    receipt.paymentStatus === "paid"
      ? receipt.paidAt || receipt.closedAt || ""
      : "";
  const hydratedStatus = receipt.paymentStatus ?? (isPaid ? "paid" : "pending");
  ...
  return {
    ...
    openedAt,
    paidAt,
    businessDate: receiptReportDateKey({
      ...receipt,
      createdAt,
      openedAt,
      paidAt,
    ...
    loyaltySyncStatus,
    loyaltyError: receipt.loyaltyError || loyaltySkipReason || "",
    loyaltySkipReason,
...
const hydrateStore = (store: Partial<StoreProfile> = {}): StoreProfile => {
  ...
  return {
    ...
    softLaunch: mergedStore.softLaunch === true,
