...
import {
  backendOrderToReceipt,
  isBackendPosOrder,
  type BackendPosOrder
...
export const getCurrentEdenAdminIdToken = async (forceRefresh = false) => {
  await edenAuthPersistence;
  const user = edenAuth.currentUser;
  if (!user) {
    throw new Error("Sign in with an Eden POS admin account first");
  }
  await requireEdenPosAccess(user);
...
export const syncReceiptToEdenOrders = async (receipt: Receipt) => {
  ...
  const openedAt = receiptOpenedAtValue(receipt) || receipt.createdAt;
  const receiptPaidAt = isPaid ? receiptPaidAtValue(receipt) : "";
  const paidAt = isPaid ? receiptPaidAt || receipt.createdAt : "";
  const reportDateTime = receiptReportDateTime({
    ...receipt,
    openedAt,
    paidAt: receiptPaidAt
  });
  const businessDate = receiptReportDateKey({
    ...receipt,
    openedAt,
    paidAt: receiptPaidAt
  });
  const businessDateLabel = new Date(`${businessDate}T00:00:00`).toLocaleDateString("th-TH");
  const receiptTimestamp = (value: string) => {
    const date = new Date(value);
    return Timestamp.fromDate(Number.isFinite(date.getTime()) ? date : new Date());
  };

  const order = {
    id: receipt.number,
    receiptNo: receipt.number,
    date: new Date(reportDateTime).toLocaleString("th-TH", {
      dateStyle: "short",
      timeStyle: "short"
    }),
    businessDate,
    ...
    openedAt: receiptTimestamp(openedAt),
    ...(isPaid ? { paidAt: receiptTimestamp(paidAt) } : {}),
...
export const loadEdenPosOrderHistory = async () => {
  const user = edenAuth.currentUser;
  if (!user) {
    throw new Error("ยังไม่ได้เข้าสู่ระบบแอดมิน Eden Cafe");
  }
  ...
  const snap = await getDocs(
    query(collection(edenDb, "orders"), orderBy("timestamp", "desc"))
  );
  const receipts: Receipt[] = [];
  const errors: string[] = [];

  snap.forEach((docSnap) => {
    const order: BackendPosOrder = {
      ...docSnap.data(),
      firestoreId: docSnap.id
    };
    if (!isBackendPosOrder(order)) return;

    const receipt = backendOrderToReceipt(docSnap.id, order);
    if (receipt) {
      receipts.push(receipt);
    } else {
      errors.push(`ข้าม order ${docSnap.id}: แปลงข้อมูล POS ไม่สำเร็จ`);
    }
  });
