...
export const usePosRegister = () => {
  ...
  const syncEdenCatalog = useCallback(async () => {
    ...
    try {
      ...
      setSyncState({
        status: "synced",
        message: `ซิงค์เมนู Eden แล้ว ${remote.products.length.toLocaleString("th-TH")} รายการ`,
  ...
  const buildLoyaltyReceiptFields = (
    ...
    const loyaltyActive =
      Boolean(details.customer?.uid && details.customer.profileSynced) &&
    ...
    return {
      ...
      loyaltyRedemption: preview.redemption,
      ...
      payableTotal: preview.payableTotal
    };
  };

  const syncReceipt = (receipt: Receipt) => {
    void syncReceiptToEdenOrders(receipt)
      .then(async (firestoreId) => {
  ...
  const buildCustomerFields = (details: BillDetails) => {
    const customer = details.customer;

    return {
      customerName:
        customer?.displayName || details.customerName || "Walk-in Customer",
      phone: customer?.phone || details.phone || "",
      ...
      customerUid: customer?.profileSynced ? customer.uid : "",
      customerEmail: customer?.email || "",
      customerLineId: customer?.lineId || "",
      customerTier: customer?.tier || "",
      customerMemberCode: customer?.memberCode || "",
      customerProfileSynced: customer?.profileSynced || false,
      note: details.note || ""
    };
  };
  ...
  const saveOpenBill = (details: BillDetails = {}): Receipt | null => {
    ...
    const existingBill = activeBillId
      ...
      : undefined;
    const customerFields = buildCustomerFields(details);
    const receipt: Receipt = {
      ...
      createdAt: existingBill?.createdAt ?? now.toISOString(),
      businessDate: existingBill?.businessDate ?? businessDate(now),
