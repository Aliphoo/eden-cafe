...
import { printSalesSummaryByCategoryReport } from "../../../integrations/posPrinter";
...
type CategoryPrintState = {
  status: "idle" | "printing" | "success" | "error";
  message: string;
};

const salesSummaryPrintErrorMessage = (error: unknown) => {
  const message = error instanceof Error ? error.message.trim() : "";
  return message
    ? `พิมพ์ไม่ได้: ${message}`
    : "พิมพ์ไม่ได้ กรุณาตรวจสอบการตั้งค่าเครื่องพิมพ์";
};
...
export const SalesSummaryScreen = ({ receipts }: SalesSummaryScreenProps) => {
  ...
  const [categoryPrintState, setCategoryPrintState] =
    useState<CategoryPrintState>({ status: "idle", message: "" });
  ...
  const receiptExportRows = useMemo<ExcelValue[][]>(
    () =>
      paidReceipts.map((receipt) => [
        receipt.number,
        formatDateTime(receiptReportDateTime(receipt)),
  ...
  const printCategoryReport = async () => {
    if (!paidReceipts.length || categoryPrintState.status === "printing") {
      return;
    }

    setCategoryPrintState({
      status: "printing",
      message: "กำลังพิมพ์..."
    });

    try {
      const printed = await printSalesSummaryByCategoryReport({
        rangeLabel,
        printedAt: new Date(),
        rows: categoryExportRows.map((row) => ({
          label: row.label,
      ...
      setCategoryPrintState({
        status: printed ? "success" : "error",
        message: printed
          ? "พิมพ์สรุปตามหมวดหมู่แล้ว"
          : "พิมพ์ไม่ได้: ไม่ได้ส่งงานพิมพ์ กรุณาตรวจสอบการตั้งค่าเครื่องพิมพ์"
      });
    } catch (error) {
      console.warn("Unable to print sales summary by category", error);
      setCategoryPrintState({
        status: "error",
        message: salesSummaryPrintErrorMessage(error)
      });
    }
  };
      ...
      <section className="sales-dashboard">
        ...
        <div className="sales-report-main">
          ...
          <section className="sales-export-panel">
            <div className="sales-export-head">
              ...
              <div className="sales-export-actions">
                ...
                <button
                  disabled={
                    !paidReceipts.length ||
                    categoryPrintState.status === "printing"
                  }
                  onClick={() => void printCategoryReport()}
                  type="button"
                >
            ...
            {categoryPrintState.message ? (
              <div className={`sales-print-status ${categoryPrintState.status}`}>
                {categoryPrintState.message}
              </div>
            ) : null}
