...
import type { PosUpdateSafety } from "../usePosRegister";
import {
  edenUserLabel,
  isEdenNativeApp,
  loadEdenAdminAccess,
...
import {
  ...
  getInstalledPosVersion,
  getPosUpdatePowerStatus,
  launchPosApkInstaller,
  openInstallUnknownAppsSettings,
  reportPosUpdateEvent,
  verifyDownloadedPosApk,
  type DownloadedPosApk,
  type InstalledPosVersion,
  type PosUpdateChannel,
  type PosUpdateCheckResult,
  type PosUpdatePowerStatus,
  type PosUpdateRelease
} from "../../../integrations/posUpdater";
...
const settingsSections: Array<{
  ...
  , { id: "updates", label: "APK Update", icon: PackageCheck }
];
...
const formatApkSize = (size: number) => {
...
export const StoreSettingsDialog = ({
  ...
  const [updateChannel, setUpdateChannel] =
    useState<PosUpdateChannel>("test");
  const [installedVersion, setInstalledVersion] =
    useState<InstalledPosVersion | null>(null);
  const [updateResult, setUpdateResult] =
    useState<PosUpdateCheckResult | null>(null);
  const [downloadedApk, setDownloadedApk] =
    useState<DownloadedPosApk | null>(null);
  const [updateBusy, setUpdateBusy] = useState(false);
  const [updateMessage, setUpdateMessage] = useState(
    "Sign in with an Eden POS admin account, then check for a protected APK update."
  );
  const [installPermissionNeeded, setInstallPermissionNeeded] =
    useState(false);
  const [powerStatus, setPowerStatus] =
    useState<PosUpdatePowerStatus | null>(null);
  ...
  useEffect(() => {
    ...
    void getInstalledPosVersion()
      .then(setInstalledVersion)
      .catch((error) => {
        setUpdateMessage(
          error instanceof Error
            ? `Unable to read installed version: ${error.message}`
            : "Unable to read installed version."
        );
      });
    void getPosUpdatePowerStatus().then(setPowerStatus);
  }, [open]);
  ...
  const updateRelease: PosUpdateRelease | null =
    updateResult?.updateAvailable === true ? updateResult : null;
  const updateWarnings = [
    ...updateSafety.warnings,
    powerStatus &&
    powerStatus.batteryLevel > 0 &&
    powerStatus.batteryLevel < 0.35 &&
    !powerStatus.isCharging
      ? `Battery is ${Math.round(powerStatus.batteryLevel * 100)}%. Plug in the POS before installing.`
      : ""
  ].filter(Boolean);
  ...
  const handleCheckUpdate = async () => {
    ...
    setDownloadedApk(null);
    setInstallPermissionNeeded(false);
    setUpdateMessage("Checking protected APK release manifest...");

    try {
      const installed = installedVersion ?? (await getInstalledPosVersion());
      setInstalledVersion(installed);
      const result = await checkPosApkUpdate({
      ...
      setUpdateMessage(
        result.updateAvailable
          ? `Update available: ${result.versionName} / versionCode ${result.versionCode}.`
          : `POS is up to date at ${installed.versionName} / versionCode ${installed.versionCode}.`
      );
    } catch (error) {
      setUpdateMessage(
        error instanceof Error
  ...
  const handleDownloadUpdate = async () => {
    ...
    if (!updateSafety.canUpdate) {
      setUpdateMessage("Update blocked until the POS safety checks are clear.");
      return;
    }
    ...
    setDownloadedApk(null);
    setInstallPermissionNeeded(false);
    setUpdateMessage("Downloading protected APK through Cloud Functions...");

    try {
      const installed = installedVersion ?? (await getInstalledPosVersion());
      setInstalledVersion(installed);
      const downloaded = await downloadPosApkRelease(updateRelease);
      const verified = await verifyDownloadedPosApk(
        downloaded,
        updateRelease.sha256
      );
      if (!verified.ok) {
        throw new Error(`APK SHA256 mismatch: ${verified.actualSha256}`);
      }

      setDownloadedApk(downloaded);
      await reportPosUpdateEvent("downloaded", {
        ...updateRelease,
        currentVersionCode: installed.versionCode,
        targetVersionCode: updateRelease.versionCode,
        size: downloaded.size,
        sha256: verified.actualSha256
      });
      const canInstall = await canRequestPackageInstalls();
      setInstallPermissionNeeded(!canInstall);
      setUpdateMessage(
        canInstall
          ? "Download complete and SHA256 verified. Ready to open Android installer."
          : "Download complete and SHA256 verified. Allow this app to install unknown apps before installing."
      );
    } catch (error) {
      const message =
        error instanceof Error ? error.message : "Download failed.";
      setDownloadedApk(null);
      setUpdateMessage(`Download failed: ${message}`);
      void reportPosUpdateEvent("failed", {
        ...updateRelease,
        targetVersionCode: updateRelease.versionCode,
        message
      }).catch(() => undefined);
  ...
  const handleInstallUpdate = async () => {
    if (!updateRelease || !downloadedApk) return;
    if (!updateSafety.canUpdate) {
      setUpdateMessage("Install blocked until the POS safety checks are clear.");
      return;
    }

    setUpdateBusy(true);
    setUpdateMessage("Preparing Android Package Installer...");

    try {
      const canInstall = await canRequestPackageInstalls();
      if (!canInstall) {
        setInstallPermissionNeeded(true);
        setUpdateMessage("Allow this app to install unknown apps before installing.");
        return;
      }
      ...
      await launchPosApkInstaller(
        downloadedApk,
        updateRelease,
        installed.versionCode
      );
      setUpdateMessage("Android installer opened. Confirm installation on the POS device.");
    } catch (error) {
      const message =
        error instanceof Error ? error.message : "Unable to open installer.";
      setUpdateMessage(`Install failed: ${message}`);
      void reportPosUpdateEvent("failed", {
        ...updateRelease,
        targetVersionCode: updateRelease.versionCode,
        message
  ...
  const handleOpenInstallSettings = async () => {
    ...
    try {
      ...
      setUpdateMessage("Return here after allowing install unknown apps for Eden Cafe POS.");
    } catch (error) {
      setUpdateMessage(
        error instanceof Error
          ? `Unable to open Android settings: ${error.message}`
          : "Unable to open Android settings."
      );
    } finally {
      setUpdateBusy(false);
    }
  ...
  const renderUpdatePanel = () => (
    <div className="settings-detail-stack pos-update-panel">
      <section className="settings-detail-section">
        <div className="settings-auth-head">
          <Smartphone aria-hidden="true" size={20} />
          <div>
            <strong>Eden POS APK</strong>
            <span>
              Current version: {installedVersion?.versionName || "-"} /
              versionCode {installedVersion?.versionCode ?? "-"}
            </span>
          </div>
        </div>

        <div className="pos-update-meta-grid">
          <div>
            <span>Package</span>
            <strong>{installedVersion?.packageName || "com.personal.pos"}</strong>
          </div>
          <div>
            <span>Runtime</span>
            <strong>{nativeApp ? "Android APK" : "Web preview"}</strong>
          </div>
          <label>
            Channel
            <select
              onChange={(event) =>
                setUpdateChannel(event.target.value as PosUpdateChannel)
              }
              value={updateChannel}
            >
      ...
      <section className="settings-detail-section">
        <div className="settings-auth-head">
          ...
          <div>
            <strong>Update safety checks</strong>
            <span>
              {updateSafety.canUpdate
                ? "Safe to download an APK update."
                : "Update is blocked until these POS tasks are clear."}
            </span>
          </div>
        </div>

        <div className="pos-update-status-grid">
          <div className={updateSafety.hasActiveCart ? "blocked" : "ready"}>
            <span>Current cart</span>
            <strong>{updateSafety.hasActiveCart ? "Pending" : "Clear"}</strong>
          </div>
          <div className={updateSafety.hasOpenBill ? "blocked" : "ready"}>
            <span>Open bills</span>
            <strong>
              {updateSafety.hasOpenBill
                ? `${updateSafety.openBillCount} open`
                : "Clear"}
            </strong>
            {updateSafety.hasOpenBill && updateSafety.openBillNumbers.length ? (
              <small>{updateSafety.openBillNumbers.join(", ")}</small>
            ) : null}
          </div>
          <div className={updateSafety.hasUnsyncedOrders ? "blocked" : "ready"}>
            <span>Order sync</span>
            <strong>{updateSafety.hasUnsyncedOrders ? "Pending" : "Synced"}</strong>
          </div>
          <div
            className={updateSafety.hasPendingLoyaltySync ? "blocked" : "ready"}
          >
            <span>Loyalty sync</span>
            <strong>
              {updateSafety.hasPendingLoyaltySync ? "Pending" : "Clear"}
            </strong>
          </div>
          <div className={updateSafety.networkOnline ? "ready" : "blocked"}>
            <span>Network</span>
            <strong>{updateSafety.networkOnline ? "Online" : "Offline"}</strong>
          </div>
        </div>

        {updateSafety.blockingReasons.length > 0 && (
          <div className="pos-update-warning">
            <AlertTriangle aria-hidden="true" size={18} />
            <ul>
              {updateSafety.blockingReasons.map((reason) => (
      ...
      <section className="settings-detail-section">
        <div className="settings-auth-head">
          ...
          <div>
            <strong>Protected release manifest</strong>
            <span>{updateMessage}</span>
          </div>
        </div>

        <div className="pos-update-actions">
          <button
          ...
          <button
            className="tool-button"
            disabled={updateBusy || !updateRelease || !updateSafety.canUpdate}
        ...
        {updateRelease ? (
          <div className="pos-update-release">
            <div>
              <span>Version</span>
              <strong>
                {updateRelease.versionName} / {updateRelease.versionCode}
              </strong>
            </div>
            <div>
              <span>Size</span>
              <strong>{formatApkSize(updateRelease.size)}</strong>
            </div>
            <div>
              <span>Force update</span>
              <strong>{updateRelease.forceUpdate ? "Yes" : "No"}</strong>
            </div>
            <div className="pos-update-sha">
              <span>SHA256</span>
              <strong>{updateRelease.sha256}</strong>
            </div>
            <div className="pos-update-notes">
              <span>Release notes</span>
              <p>{updateRelease.releaseNotes || "No release notes."}</p>
            </div>
          </div>
        ) : (
          updateResult && (
            <div className="pos-update-ready">
              <CheckCircle2 aria-hidden="true" size={18} />
              No newer APK release is active for this channel.
            </div>
          )
        )}
        ...
        {downloadedApk && (
          <div className="pos-update-ready">
            <CheckCircle2 aria-hidden="true" size={18} />
            Downloaded {formatApkSize(downloadedApk.size)} and verified SHA256.
          </div>
        )}
      </section>
    </div>
    ...
    <div className="settings-detail-stack">
      ...
      <section className="settings-detail-section">
        ...
        <div className="settings-form-grid">
          ...
          <label className="settings-field settings-check-field">
            <input
              checked={form.softLaunch === true}
              onChange={(event) =>
                setForm((current) => ({
                  ...current,
                  softLaunch: event.target.checked
                }))
              }
              type="checkbox"
            />
            <span>
              <strong>Soft launch mode</strong>
              <small>Skip POS loyalty sync and write loyaltyError=soft-launch.</small>
            </span>
          </label>
          <label className="settings-field">
            PromptPay ID
  ...
  const renderActivePanel = () => {
    ...
    if (activeSection === "updates") return renderUpdatePanel();
