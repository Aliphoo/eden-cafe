  ...
  const loyaltyPreview = useMemo(
    () =>
      calculateLoyaltyPreview({
        ...
        requestedRedeemPoints: Number(redeemPoints || 0),
        subtotal,
        totalBeforeLoyalty: total
      }),
    [cart, discount, loyaltyConfig, redeemPoints, selectedCustomer, subtotal, total]
  );
        ...
        <div className="payment-content eden-payment-content">
          ...
          <section className="loyalty-payment-panel" aria-label="Loyalty points">
            ...
            <label>
              ใช้แต้ม
              <input
                disabled={!selectedCustomer?.profileSynced || !loyaltyConfig.enabled}
                inputMode="numeric"
                min="0"
                onChange={(event) =>
                  setRedeemPoints(event.target.value.replace(/\D/g, ""))
                }
                placeholder={loyaltyConfig.enabled ? "0" : "ยังไม่เปิดใช้ Loyalty"}
                type="text"
                value={redeemPoints}
              />
            </label>
