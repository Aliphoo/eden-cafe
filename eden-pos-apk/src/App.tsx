...
import { POS_BUILD_VERSION_LABEL } from "./buildInfo";
...
import { reportPendingPosInstallIfComplete } from "./integrations/posUpdater";
...
const PosApp = () => {
  ...
  useEffect(() => {
    void reportPendingPosInstallIfComplete().catch((error) => {
      console.warn("Unable to report pending POS APK install", error);
    });
  ...
  const todayTotal = useMemo(() => {
    ...
    return pos.data.receipts
      .filter(
        (receipt) =>
          receiptReportDateKey(receipt) === today &&
  ...
  const historySyncBusy = pos.historySyncState.status === "syncing";
  const historySyncDisabled = historySyncBusy || !authChecked || !adminAccount;
  const historySyncLast = pos.historySyncState.lastSyncedAt
    ? new Date(pos.historySyncState.lastSyncedAt).toLocaleString("th-TH", {
        dateStyle: "short",
        timeStyle: "short"
      })
    : "";
        ...
        <div className="drawer-layer">
          ...
          <aside className="app-drawer" aria-label="เมนูหลัก">
            ...
            <nav className="drawer-nav">
              ...
              <button
                className={historySyncBusy ? "syncing" : ""}
                disabled={historySyncDisabled}
                onClick={() => void pos.syncReceiptHistory()}
                title={
                  !authChecked
                    ? "กำลังตรวจสอบบัญชีแอดมิน"
                    : !adminAccount
                      ? "ล็อกอินแอดมินก่อนซิงค์ประวัติ"
                      : "ซิงค์ประวัติ POS กับหลังบ้าน"
                }
                type="button"
              >
            ...
            <div className={`drawer-history-sync ${pos.historySyncState.status}`}>
              <strong>ประวัติรายการ</strong>
              <span>{pos.historySyncState.message}</span>
              {historySyncLast && <small>ล่าสุด {historySyncLast}</small>}
              {pos.historySyncState.errors?.length ? (
                <small>{pos.historySyncState.errors[0]}</small>
              ) : null}
            </div>

            <div className="drawer-version">{POS_BUILD_VERSION_LABEL}</div>
          </aside>
        </div>
      )}
